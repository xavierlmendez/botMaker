...
class PolynomialRegressionExpander(HypothesisExpander):
    ...
    def expand(self, valueArray):
        ...
        featureSpaceExpandedToDegree = (
            expandedWeightsArray[:, :, None] ** exponents[None, None, :]
        ...
        featureSpaceExpandedToDegree = featureSpaceExpandedToDegree.reshape(
            featureSpaceExpandedToDegree.shape[0],
            featureSpaceExpandedToDegree.shape[1] * self.degree,
        )
    ...
    def expandHypothesis(
        self, initialArray: np.ndarray
    ):  # can be used to shape both hypothesis function and features
        return self.expand(initialArray).reshape(-1)

    def fitDataToHypothesis(
        self, data: np.ndarray, needsReshape=False
