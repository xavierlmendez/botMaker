...
hypothesisSpaceExpansion3rdDegree = hypothesisSpaceExpansion.reshape(
    -1, 1
) @ hypothesisSpaceExpansion.reshape(1, -1)
...
for feature in initialArray:
    for degree in range(2):
        expandedHypothesis[index] = (
            feature ** (degree + 1)
        )  # for hypothesis function should result in the initial weights being scaled the same but will need to see if this affect model performance
        index += 1
