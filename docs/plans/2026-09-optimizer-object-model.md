...
| # | Assumption | Why |
|---|---|---|
| A-1 | `tests/ml/test_training_baseline.py` and `tests/ml/test_nystrom_search_baseline.py` stay byte-identical in every slice, the docs slice included as the check that they pass on the branch. | They are the only guarantee that a restructuring changed no number (CONTRIBUTING, refactor rule). |
...
| 1 | `refactor/optimizer-object-model-1` | `RegularizationFunction` becomes an `ABC` with `compute_penalty(parameters)`; `CollisionPenalty`, `AdjacencyPenalty` (laplacian and edge-product forms as one class with a form knob, or two classes — pick by P-1 and say which) and `DiversityPenalty` implement it in torch under `math/algorithms/two_hot_span/`, each weight a knob on its class with today's default; `training_loss` sums injected penalties instead of branching. | Each penalty equals today's inline term on a fixed V to the last bit; a numeric gradient check against autograd per penalty; `test_task_kind.py` and `test_smoke_modules.py` updated for the abstract base; torch tests `importorskip`. | BL-48 progress line; LEARNING_LOG entry "a penalty as an injected object". | Both baselines byte-identical; every existing two-hot test passes unchanged in what it asserts. |
