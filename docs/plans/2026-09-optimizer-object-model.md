...
- **Two measures exist twice by accident.** The collision measure is `collision_measure`
  ...
  (`:237-239`). Same arithmetic, two array libraries — corrected 2026-09-11 (slice 2): the collision
  measure is *not* the same arithmetic, the numpy one guards the zero vector and scales by the peak
  entry first, so only the Laplacian is a true duplicate.
...
| 2 | `refactor/optimizer-object-model-2` | `CostFunction` redefined as `compute_cost(parameters)` with an optional `task_kind`; the projector as one concept with two arithmetics — an abstract projector, `ExactProjector` (numpy, `pinv`, reporting) and `RidgeProjector` (torch, `epsilon` a knob, training); `SpanCost(projector, X)` as the first `AbstractCostFunction` (one cost class, the projector injected: not a `RidgeSpanCost` beside an `ExactSpanCost`); the collision measure keeps two arithmetics by design (the numpy one guards the zero vector and scales by the peak entry before squaring, the torch term is on the gradient path); the Laplacian dedupe moves to slice 4 (D − A and X Xᵀ differ by ulps, and the spectral start amplifies ulps, BL-50; slice 4 is the slice that regenerates fixtures). | `tests/math/test_projector.py`: `ExactProjector` reproduces `projector_residual` bit for bit against the pasted old body, residual invariances by hypothesis, `SpanCost` equality; `tests/math/algorithms/test_two_hot_span_projectors.py`: `RidgeProjector` reproduces the old inline ridge bit for bit, the filter-factor closed form, ε → 0, a dependent column, gradcheck; the D-31 guards that the reporting path never sees `epsilon` stay on `ExactProjector`; `test_task_kind.py` rewritten for the new contract. | LEARNING_LOG entry "the ridge projector as a filter-factor smoother" (σ²/(σ²+ε)); D-31 unchanged. | Both baselines byte-identical; one definition of the projector per arithmetic under `src/`; the collision measure keeps its two arithmetics by design and the Laplacian dedupe is slice 4's. |
| 3 | `refactor/optimizer-object-model-3` | An `AbstractStepRule` (`bind` once per run, `learning_rate_in_force`, `zero_gradient`, `step`) with `AdamStepRule` holding the learning rate and its schedule as knobs, and an `AbstractLearningRateSchedule` with one object per form — `ConstantSchedule`, `LinearSchedule`, `CosineSchedule`, `WarmupCosineSchedule`, the four `LEARNING_RATE_SCHEDULES` names — taking the horizon per call; `describe()` reads the knobs off each. | Each schedule's multiplier sequence equals today's `learning_rate_lambda` exactly over 1 000 steps for the default and one non-default knob setting; an Adam step equals `torch.optim.Adam` plus `LambdaLR` on one seed, bit for bit; `describe(AdamStepRule(...))["params"]` names the learning rate. | LEARNING_LOG entry "step rules with schedules"; BL-48 progress line. | Both baselines byte-identical; `learning_rate_lambda` deleted (its four bodies are the schedules' `multiplier`s, pinned against the pasted closure). |
| 4 | `refactor/optimizer-object-model-4` | **Shipped 2026-09-11.** `AbstractOptimizer` in `math/algorithms/abstract_optimizer.py` owning `run()` (one per optimizer, refused twice), the recorder guard, the stop check and the end record, with `_begin`, `_step`, `_iterate`, `_assemble` and `_recorder_extras` as the seams, mirroring `AbstractGraphAlgorithm`; `Stop`/`StopReason` and `OptimizerResult` (parameters, final training loss, steps taken, stop reason and detail, configuration); `TwoHotSpanProblem` in the problem module (graph, K, X, the graph's own Laplacian and adjacency, `constraint_vector` = ones as BL-41's seam, `report` through the exact arithmetic); `TwoHotSpanOptimizer` in `math/algorithms/two_hot_span/optimizer.py` taking the problem, a cost, penalties, a step rule and a recorder, with `TwoHotSpanSettings` (step count, seed, normalisation, zero-sum tolerance) as the loop's own knobs and `TwoHotSpanResult` per P-9; `configuration` assembled through `describe.configuration_of` (P-4), epsilon and every learning rate in it; the two exceptions deleted, a stopped run returns with its reason; `training_cost(problem)` and `project_orthogonal` beside it; `mllib.ml.projects.two_hot_span_composition.compose_two_hot_span` as the composition roots' one home for names and knobs (P-8), used by the harness, the stress ladder and the three examples, each building fresh objects per cell (P-5); the two-hot recorder derives `loss_history` and `learning_rate_history` from its own frames and carries the run's `configuration`; `two_hot_span_optimizer.py`, `TwoHotSpanConfig` and `fit_two_hot_span` **removed** (every caller is a composition root). The Laplacian dedupe is **not** in this slice: the two derivations differ by ulps the spectral start amplifies (BL-50), and this slice regenerated no number, so the collapse waits for the fixture regeneration BL-50 decides (ARCHITECTURE §5). The collision measure keeps its two arithmetics (slice 2). | The refactor snapshot byte-identical through a `HistoryRecorder` that reads the per-step losses and learning rates off the recorder; `test_abstract_optimizer.py` (the loop, stops, the guard, one run per optimizer); `test_two_hot_span_optimizer_contract.py` (`project_orthogonal` is the old `_project_zero_sum` to the last bit, settings, the JSON-ready configuration, a constraint stop, one run); `test_two_hot_span_problem_object.py`; `configuration_of`; every two-hot test module composing the optimizer directly; the recorder's histories off its frames and a stopped run's end frame; the harness rows and stress cells carrying `configuration` and a `stopped` status; the walkthrough fixture and the three stress report fixtures regenerated with every numeric leaf unchanged (verified before overwriting) and the new keys named in the PR. | BL-48 progress line; LEARNING_LOG "An optimizer as an algorithm object"; ARCHITECTURE §1, §2 (`AbstractOptimizer` and `TwoHotSpanProblem` rows, no longer target), §4, §5, §6; CONTEXT.md gains **Stop reason**; this row and §5's acceptance sentences. | Both baselines and the refactor snapshot byte-identical; the acceptance sentences in §5 checked by the reviewer-agent; no reference to the deleted module outside dated history. |
| 5 | `refactor/optimizer-object-model-5` | **Shipped 2026-09-11.** Fix slice. `math/hypothesis.py:51` calls `hypothesis_expander.expand(self.hypothesis, self.degree)`, a signature no expander defines; fix to the expander contract the descent base already uses. | A test that calls `HypothesisFunction.expand_hypothesis` through each expander and asserts the shape; the training baseline byte-identical. | BL-49 amended to say the bug is closed here. | Both baselines byte-identical; the test fails on `main` @ `4c76986` and passes on the branch. |
...
| A refactor that reads as the same arithmetic is not the same arithmetic. | It happened in slice 3: `math.pi * (step / last_step)` rounds differently from `math.pi * step / last_step`, and the exact refactor snapshot caught the one-ulp change at step 14 of the cosine cell. The snapshot, not a reading of the diff, is the proof of "bit-identical"; every slice runs it before its tests are written. |

## 5. Immediate next actions

1. **Shipped 2026-09-11.** Slices 0 and 1 merged (#43, #44); slices 2–5 stacked on
   `refactor/optimizer-object-model-5` for one PR. BL-48 is closed on merge; BL-49 (the descent
   stack), BL-50 (the spectral-start fixtures and, with them, the Laplacian's two derivations) and
   BL-46 / BL-41 open against the seams §3 names.
2. **After the merge.** Delete the five stacked branches; `docs/plans/2026-09-two-hot-span.md`
   stays as the dated record of the prototype whose module this plan replaced.
3. **Acceptance sentences**, checked by the reviewer-agent at slice 4 (2026-09-11) and again when
   BL-46 and BL-41 open: "BL-46's restarts are a composition root that builds N fresh optimizers
   from different seeds or starts and selects among their results, editing no class"; "BL-46's
   joint (W, H) factorization is a new `AbstractOptimizer` subclass filling `_begin`, `_step`,
   `_iterate` and `_assemble`, reusing the problem, the projector, the penalties, the step rule
   and the recorder, and editing no existing class"; "BL-41's ncut is a `TwoHotSpanProblem`
   subclass whose `constraint_vector` is sqrt(d), with an `AbstractCostFunction` for its
   objective if the span cost does not serve, and edits no existing class".
